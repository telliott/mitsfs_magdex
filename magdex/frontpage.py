...
SECTIONS = {"library_front": "The Big Guns",
			"library_small": "Small Magazines",
			"library_large": "Large Magazines",
			...
			"foreign_one_offs.json": "One-Off Issues of Foreign Publications (in Annex)"
			}
...
def tableOfContents(section, root, link_template):
	...
	column_length = ceil(len(magazines) / COLUMNS)
	columns = [magazines[i:i + column_length] \
					for i in range(0, len(magazines), column_length)]
	
	tagged_columns = []
	if raw_display:
		tagged_columns = [outputTextColumn(column) for column in columns]
	else:	
		tagged_columns = [outputColumn(section, column, link_template) \
							for column in columns]
	
	q = "<h2>%s</h2>\n" % SECTIONS[section]
	q += "<table width=\"100%\"><tr>\n"
...
"""
Creates an individual column list of magazines
"""
def outputColumn(section, column, link_template):
	return "<ul>%s</ul>" % "\n".join([create_entry(link_template, section, mag) for mag in column])

"""
Creates the text for unlinked magazines
"""
def outputTextColumn(column):
	q = ""
	for mag in column:
